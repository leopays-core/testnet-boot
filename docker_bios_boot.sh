...
echo ===== Start: $step ============ >> $logfile
echo executing: DCcli --wallet-url $wdurl --url http://$bioshost:$biosport push action lpc.token create '[ "lpc", "1000000000.0000 LPC" ]' -p lpc.token@active | tee -a $logfile
echo executing: DCcli --wallet-url $wdurl --url http://$bioshost:$biosport push action lpc.token issue '[ "lpc", "20000000.0000 LPC", "init issue" ]'  -p lpc@active | tee -a $logfile
echo executing: DCcli --wallet-url $wdurl --url http://$bioshost:$biosport push action lpc.token transfer '[ "lpc", "leopays", "20000000.0000 LPC", "For LeoPays" ]' -p lpc@active | tee -a $logfile
echo ----------------------- >> $logfile

$DCcli --wallet-url $wdurl --url http://$bioshost:$biosport push action lpc.token create '[ "lpc", "1000000000.0000 LPC" ]' -p lpc.token@active >> $logfile 2>&1
$DCcli --wallet-url $wdurl --url http://$bioshost:$biosport push action lpc.token issue '[ "lpc", "20000000.0000 LPC", "init issue" ]' -p lpc@active >> $logfile 2>&1
$DCcli --wallet-url $wdurl --url http://$bioshost:$biosport push action lpc.token transfer '[ "lpc", "leopays", "20000000.0000 LPC", "For LeoPays" ]' -p lpc@active >> $logfile 2>&1
echo ==== End: $step ============== >> $logfile
step=$(($step + 1))
...
sleep 3
$DCcli --wallet-url $wdurl --url http://$bioshost:$biosport push action lpc init '[0, "4,LPC"]' -p -p lpc@active >> $logfile 2>&1
echo ==== End: $step ============== >> $logfile
step=$(($step + 1))
